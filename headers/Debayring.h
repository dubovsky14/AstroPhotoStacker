...
namespace AstroPhotoStacker {
    ...
    template<typename output_type = short int>
    std::vector<std::vector<output_type>> debayer_raw_data(const std::vector<output_type> &data_original, int width, int height, const std::array<char, 4> &bayer_pattern)  {
    ...
    void debayer_monochrome(std::vector<ValueType> *data, int width, int height, const std::array<char, 4> &bayer_pattern) {
        for (int y = 0; y < height-1; y++) {
            for (int x = 0; x < width-1; x++) {
