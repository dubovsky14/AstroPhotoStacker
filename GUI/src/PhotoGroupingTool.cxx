...
void PhotoGroupingTool::run_grouping()  {
    ...
    if (!m_group_by_files)   {
        ...
        // group by files
        std::map<std::string, vector<size_t>> groups_map;
        for (size_t i = 0; i < m_photos.size(); i++) {
            const std::string &file_address = m_photos[i].input_frame.get_file_address();
            groups_map[file_address].push_back(i);
        }

        for (const auto &group_pair : groups_map) {
            m_groups.push_back(group_pair.second);
        }
    }
